...
void ConnectPool::recycle_connection() {
    while (true)
    {
        ...
        while (mysql_conn_que_.size() > min_conn_)  // 只有当前连接数大于最小连接数时才销毁连接
        {
            ...
            if (conn->get_alive_time() >= max_Idle_time)  // 如果连接的空闲时长超过了最大空闲时长 -> 销毁
            {
                mysql_conn_que_.pop();
                delete conn;
            }
...
bool ConnectPool::parse_json_file() {
    std::ifstream ifs("../config.json");    // 读取 配置文件
    std::string json_str;
    while (ifs) {
        std::string line;
    ...
    std::string err_msg;
    myJson::Json json = myJson::Json::parse(json_str, err_msg);     // 使用自定以的myJson解析器解析
    if (err_msg == "" && json.isObject()) {
        ...
        min_conn_ = json["min_conn"].toNumber();
        time_out_ = json["time_out"].toNumber();
        max_Idle_time = json["max_Idle_time"].toNumber();
        return true;
    }
    return false;
}